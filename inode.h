...
class inode_state {
   ...
   public:
      ...
      wordvec getPath()     {return path;}
      string getPrompt()    {return prompt;}
      void setPrompt(string p) { prompt = p; }
      void setCwd(inode_ptr n) { cwd = n; }
      void setPath(wordvec v)  { path = v; }
};
...
class inode {
   ...
   public:
      ...
      file_base_ptr getContents()  {return contents;}
};
...
class file_base {
   ...
   public:
      ...
      friend void print_recursive( directory_ptr D, wordvec pth);
};
...
class directory: public file_base {
 private:
  ...
  directory_ptr parent {nullptr};
  void setDirents(map<string, inode_ptr> d) {dirents = d;}
 public:
  size_t size() const override;
  void remove (const string& filename);
  wordvec getDirents();
  directory_ptr getParent() {return parent;}
  void setParent(directory_ptr p) {parent = p;}
  map<string, inode_ptr>& getMap();
  inode& mkdir (inode_state& state, const string& dirname);
